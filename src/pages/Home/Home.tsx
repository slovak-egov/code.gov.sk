...
const Home = () => {
  ...
  const debouncedChangeHandler = useMemo(() => debounce(changeHandler, 750), []);

  const dataset = useGetDataset(params);

  return (
    <AppLayout title={'code.gov.sk'}>
      <div className="govuk-width-container">
        <main className="govuk-main-wrapper govuk-main-wrapper--auto-spacing">
                ...
                <div className="idsk-intro-block">
                  <div className="govuk-grid-row">
                    <div className="govuk-grid-column-full govuk-grid-column-two-thirds-from-desktop">
                      <h2 className="govuk-heading-xl">{t('home.title')}</h2>
                      <div
                        data-module="idsk-search-component"
                        className="idsk-search-component idsk-search-component--small"
                      >
                        <label
                          className="idsk-search-component__label--small govuk-visually-hidden"
                          htmlFor="codeSearch"
                          style={{ display: 'none' }}
                        >
                        ...
                        <input
                          className="govuk-input idsk-search-component__input idsk-search-component__input--small"
          ...
          <div className="govuk-grid-row">
            <SideFilters />
            <SearchResults
              loading={dataset.isLoading || dataset.isFetching}
              error={dataset.error as Error}
              data={dataset.status === 'loading' ? [] : dataset?.data?.results || []}
              totalCount={dataset?.data?.total || 0}
            />
          </div>
        </main>
      </div>
